...
ENABLE_UPLOADS = os.environ.get("ENABLE_UPLOADS", "1").strip().lower() in (
    "1",
    "true",
    "yes",
)
...
API_URL = os.environ.get("API_URL", "https://mizube.oud.ai/api/users/DemoDashboard")
...
def persist_frame_to_s3(frame, class_name: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        LOGGER.exception(
            "S3_UPLOAD_FAILED bucket=%s key=%s error=%s", AWS_BUCKET, filename, exc
        )
    ...
    def add_detection(self, frame, detection: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            if (
                ...
                and self._pending_local_path != image_info["local_path"]
            ):
...
class VideoReader:
    ...
    def __init__(self, video_path, resize_percent=65):
        ...
        LOGGER.info(
            f"Properties: {self.width}x{self.height} @ {self.fps}FPS, {self.total_frames} frames"
        )
...
class PerformanceTracker:
    ...
    def print_summary(self):
        ...
        LOGGER.info(
            f"Performance Summary - Frames: {stats['total_frames']}, "
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(
                Fore.GREEN
                + f"✓ Model loaded on {DEVICE}: {', '.join(model.names.values())}"
            )
        except Exception as e:
        ...
        if SAVE_OUTPUT_VIDEO:
            output_path = os.path.join(
                RESULTS_DIR, f"output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
            )
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")
            fps = video_reader.fps if video_reader.fps > 0 else 30
            frame_size = (
                int(video_reader.width * RESIZE_PERCENT / 100),
        ...
        while True:
            ...
            if results[0].boxes is not None and results[0].boxes.cls is not None:
                ...
                for i in range(num_detections):
                    ...
                    track_id = (
                        int(track_list[i])
                        if track_list is not None and i < len(track_list)
                        else None
                    )
            ...
            annotated_frame = draw_detections(
                frame, results[0], model.names, fps=stats["current_fps"]
            )
            ...
            print(
                f"\rFrame {perf.frame_count}/{video_reader.total_frames} "
            ...
            if show_window:
                try:
                    ...
                    print(
                        Fore.YELLOW + "Continuing in headless mode (no video display)\n"
                    )
                    LOGGER.warning(
                        "OpenCV GUI not available; continuing without display"
                    )
        ...
        if SAVE_METADATA:
            metadata_file = os.path.join(
                RESULTS_DIR, f"metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
